...
def create_buttons_and_labels(App):
    ...
    # Main info
    create_text(App, 'current_clicks', 30, App.first_background_color, "Kappas:")
    ...
    # CLICKS
    create_text(App, 'click', 20, App.first_background_color, "Power of Kappa:")
    create_variable(App, 'click', App.one_click, 20, App.first_background_color)

    # AUTO_CLICKS
    create_text(App, 'auto_click', 20, App.first_background_color, "Kappas per sec:")
    create_variable(App, 'auto_click', App.auto_click, 20, App.first_background_color)

    # CLICK BUTTON
    path = 'pictures/Kappa.png'
    setattr(
        App,
        'Kappa_image',
        PhotoImage(file=path)
    ...
    setattr(
        App,
        'click_button',
        Button(App.master, bg='#660066', activebackground='#993399', borderwidth=5, image=getattr(App, 'Kappa_image'))
    )
    getattr(
        App,
        'click_button'
    ).bind('<Button-1>', lambda event: App.update("CLICK"))

    # Auto-clickers buttons
